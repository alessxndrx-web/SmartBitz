...
describe('Transactional Flow Integration (Simple)', () => {
  ...
  describe('Complete Transactional Flow', () => {
    ...
    it('should execute complete flow: Purchase → Inventory → Invoice → Payment', async () => {
      ...
      const stockAfterPurchase = await client.inventoryItem.findUnique({
        where: { id: inventoryItemId },
      });
      expect(stockAfterPurchase.quantity).toBe(20);

      const movementsAfterPurchase = await client.inventoryMovement.findMany({
      ...
      const stockAfterInvoice = await client.inventoryItem.findUnique({
        where: { id: inventoryItemId },
      });
      expect(stockAfterInvoice.quantity).toBe(15);

      const movementsAfterInvoice = await client.inventoryMovement.findMany({
        where: { itemId: inventoryItemId, tenantId },
      });
      ...
      const finalMovements = await client.inventoryMovement.findMany({
        where: { itemId: inventoryItemId, tenantId },
      ...
      const finalStock = await client.inventoryItem.findUnique({
        where: { id: inventoryItemId },
      });
    ...
    it('should maintain tenant isolation throughout the flow', async () => {
      ...
      const tenant2 = await client.tenant.create({
        data: {
          name: 'Second Company',
          slug: `second-company-${Date.now()}`,
          ruc: '987654321',
          businessType: 'RETAIL' as any,
          subscriptionPlan: 'BASIC' as any,
        },
      ...
      const tenant2Items = await client.inventoryItem.findMany({
        where: { tenantId: tenant2.id },
      });
      expect(tenant2Items).toHaveLength(0);

      // Verify first tenant's data is isolated
      const tenant1Items = await client.inventoryItem.findMany({
        where: { tenantId },
      });
